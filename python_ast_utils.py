...
def tree_to_ast(tree):
    ...
    if tree._label == 'Expr' or tree._label == 'Expr':
...
def parse_asts(srcs, filter_uniques = False, ids = None, ignore_syntax_errors = False, verbose = False, flist = []):
    ...
    for i in progbar(range(len(srcs))):
        try:
            ...
            if ignore_syntax_errors:
                ...
                print("Removing--------"+str(flist[i-1])+"\n")
                os.remove("mock_dataset\\trace 12\\"+str(flist[i-1]))
                #print(srcs[i][:500])
                continue
